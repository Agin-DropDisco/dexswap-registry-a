...
/// @title dexDAO Token Multi-Registry
/// @notice Maintains multiple token lists, curated by the DAO
contract DEXTokenRegistry is Ownable {
    ...
    function addTokens(uint256 _listId, address[] memory _tokens) public onlyOwner {
        require(_listId <= listCount, 'DEXTokenRegistry : INVALID_LIST');
    ...
    function getTokensRange(
        ...
        require(
            ...
            'DEXTokenRegistry : INVALID_RANGE'
        );
        require(_start <= _end, 'DEXTokenRegistry : INVALID_INVERTED_RANGE');
